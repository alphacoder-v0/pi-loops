...
// `_meta.pi_dedup_key` (so every beat counts as a distinct trigger) and a human-readable
// `_meta.pi_summary` (what the audit and the chat see).
...
rl.on("line", (line) => {
	...
	if (msg.method === "initialize") {
		...
		setInterval(() => {
			beats++;
			send({ jsonrpc: "2.0", method: "notifications/pi/demo/heartbeat", params: { _meta: { pi_dedup_key: `beat-${process.pid}-${beats}`, pi_summary: `demo heartbeat #${beats} at ${new Date().toISOString()}` } } });
		}, 10_000).unref?.();
