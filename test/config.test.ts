...
test("envFlag: 1 or true, and a typo is off", () => {
	const name = "LOOPS_TEST_FLAG";
	const restore = process.env[`PI_${name}`];
	const set = (value?: string) => {
		if (value === undefined) delete process.env[`PI_${name}`];
		else process.env[`PI_${name}`] = value;
	};
	try {
		set(undefined);
