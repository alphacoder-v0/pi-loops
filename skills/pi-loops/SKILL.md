...
5-field cron (local time), `hourly` / `daily` / `weekly`, crontab's `@hourly` / `@daily` /
`@weekly` / `@monthly`, `every 30m`, `in 10m`, `at 2026-09-08T18:00`. `daily` means 09:00 local;
`@daily` means midnight.
