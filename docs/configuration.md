...
| `PI_LOOPS_CHILD=1` | set on sub-agents by pi-loops; keeps the extension dormant there |
...
(allowlist for the sub-agent), `--timeout 20m` (default 15m), `--no-catchup`, `--stateful`,
